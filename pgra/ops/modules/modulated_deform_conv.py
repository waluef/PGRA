...
class ModulatedDeformConv(nn.Module):
    def __init__(self, in_channels, out_channels,
        ...
        if out_channels % groups != 0:
            raise ValueError(
                "out_channels {} must be divisible by groups {}".format(out_channels, groups))
    ...
    def forward(self, input, offset, mask):
        assert (2 * self.deformable_groups *
                self.kernel_size[0] * self.kernel_size[1] == offset.shape[1])
        assert (self.deformable_groups *
                self.kernel_size[0] * self.kernel_size[1] == mask.shape[1])
...
class ModulatedDeformConvPack(ModulatedDeformConv):
    def __init__(self, in_channels, out_channels,
                 kernel_size, stride, padding,
                 dilation=1, groups=1, deformable_groups=1,
                 im2col_step=64, bias=True, lr_mult=0.1):
